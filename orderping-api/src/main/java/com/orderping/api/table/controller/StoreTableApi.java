...
public interface StoreTableApi {
    ...
    ResponseEntity<StoreTableResponse> createStoreTable(StoreTableCreateRequest request);
    ...
    @Operation(summary = "매장별 테이블 목록", description = "매장 ID로 테이블 목록을 조회합니다")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    ResponseEntity<List<StoreTableResponse>> getStoreTablesByStoreId(
        @Parameter(description = "매장 ID", required = true) Long storeId
    );

    @Operation(summary = "매장별 상태별 테이블 목록", description = "매장 ID와 상태로 테이블 목록을 조회합니다")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    ...
    @Operation(summary = "테이블 상태 변경", description = "테이블 상태를 변경합니다")
    @ApiResponses({
    ...
    @Operation(summary = "테이블 삭제", description = "ID로 테이블을 삭제합니다")
    @ApiResponses({
    ...
    @Operation(summary = "테이블 비우기", description = "현재 테이블을 종료하고 같은 번호의 새 테이블을 생성합니다")
    @ApiResponses({
