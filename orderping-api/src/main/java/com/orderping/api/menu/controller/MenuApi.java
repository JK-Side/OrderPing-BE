...
public interface MenuApi {
    ...
    @Operation(summary = "메뉴 목록 조회", description = "storeId, categoryId로 메뉴 목록을 조회합니다 (본인 매장만 가능). 둘 중 하나는 필수이며, 둘 다 입력하면 해당 매장의 해당 카테고리 메뉴만 조회됩니다.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "조회 성공"),
    ...
    @Operation(summary = "판매 가능한 메뉴 목록", description = "매장의 판매 가능한 메뉴 목록을 조회합니다 (본인 매장만 가능)")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "조회 성공"),
        @ApiResponse(responseCode = "403", description = "본인 매장이 아님")
    })
